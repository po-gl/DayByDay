...
struct DayData {
  ...
  @discardableResult
  static func addDay(
    activeFor category: StatusCategory,
    date: Date,
    context: NSManagedObjectContext
  ) -> DayMO {
    let newDay = DayMO(context: context)
  ...
  @discardableResult
  static func addDay(
    active: Bool, creative: Bool, productive: Bool,
    date: Date,
    context: NSManagedObjectContext
  ) -> DayMO {
    let newDay = DayMO(context: context)
  ...
  static func saveContext(
    _ context: NSManagedObjectContext, errorMessage: String = "CoreData error."
  ) {
    do {
      try context.save()
    } catch {
  ...
  static func getDay(for date: Date, days: [DayMO]) -> DayMO? {
    for day in days {
      if isDayOnDate(day, date: date) { return day }
    }
    return nil
