...
private struct ButtonClusterForDay: View {
  ...
  @ViewBuilder
  private func SwirlButton(
    _ text: String, for category: StatusCategory, startAngle: AngleStart, _ geometry: GeometryProxy
  ) -> some View {
  ...
  private func WaveMask(_ geometry: GeometryProxy) -> some View {
    ZStack {
      ...
      ZStack {
        WigglyBar(
          category: .active, width: 30, height: 400, frequency: 50, amplitude: 5, speed: 2.0
        )
        .frame(width: 200, height: 400)
        .rotationEffect(.degrees(90))
        .shadow(radius: 22)
        WigglyBar(
          category: .active, width: 30, height: 400, frequency: 50, amplitude: 5, speed: 0.8
        )
        .frame(width: 200, height: 400)
        .rotationEffect(.degrees(90))
        .shadow(radius: 15)
  ...
  private func offset(for startAngle: AngleStart, _ geometry: GeometryProxy) -> CGPoint {
    ...
    case .topRight:
      return CGPoint(
        x: max(min(-scrollOffset, bound), 0.0), y: max(-scrollOffset * (factor - 0.2), 0.0))
    case .bottom:
      let bottomThreshold = -66.0
      return scrollOffset < bottomThreshold
        ? CGPoint(x: 0.0, y: (-scrollOffset + bottomThreshold) * (factor - 0.6)) : CGPoint()
    case .top:
      return CGPoint()
    }
  }
