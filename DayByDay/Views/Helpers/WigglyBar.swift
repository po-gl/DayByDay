    ...
    ZStack {
      ZStack {
        WigglyPath(frequency: frequency, amplitude: amplitude)
          .stroke(
            LinearGradient(for: category),
            style: StrokeStyle(lineWidth: width - amplitude * 2, lineCap: .round)
          )
        ...
        VStack {
          VerticalLine()
            .stroke(
              LinearGradient(for: category), style: StrokeStyle(lineWidth: width, lineCap: .round)
            )
