...
final class DateCompareTests: XCTestCase {
  ...
  func testIsSameDay() throws {
    ...
    XCTAssertTrue(
      date.hasSame(.day, as: nextMonthDate),
      "Expected true, The dates should both be the 4th of the month.")
  ...
  func testIsToday() throws {
    ...
    XCTAssertFalse(
      date.isToday(), "Expected false. Start of day subtracted by 1 second should not be today.")
  ...
  func testAtHour() throws {
    ...
    XCTAssertEqual(
      calendar.component(.hour, from: firstHourDate),
      calendar.component(.hour, from: calendar.date(from: DateComponents(hour: 0))!),
      "Expected 0 hour")
    ...
    XCTAssertEqual(
      calendar.component(.hour, from: lastHourDate),
      calendar.component(.hour, from: calendar.date(from: DateComponents(hour: 23))!),
      "Expected 23 hours")

    let pastDayHourDate = date.atHour(50)
    XCTAssertTrue(date.isSameDay(as: pastDayHourDate))
    XCTAssertEqual(
      calendar.component(.hour, from: pastDayHourDate),
      calendar.component(.hour, from: calendar.date(from: DateComponents(hour: 23))!),
      "Expected 23 hours. atHour() should keep within the same day")

    let negativeHourDate = date.atHour(-4)
    XCTAssertTrue(date.isSameDay(as: negativeHourDate))
    XCTAssertEqual(
      calendar.component(.hour, from: negativeHourDate),
      calendar.component(.hour, from: calendar.date(from: DateComponents(hour: 20))!),
      "Expected 20 hours. Starting from 24 hours minus 4")

    let negativePastHourDate = date.atHour(-30)
    XCTAssertTrue(date.isSameDay(as: negativePastHourDate))
    XCTAssertEqual(
      calendar.component(.hour, from: negativePastHourDate),
      calendar.component(.hour, from: calendar.date(from: DateComponents(hour: 0))!),
      "Expected 0 hours.")
    ...
    XCTAssertEqual(
      calendar.component(.hour, from: negativeBarelyPastHourDate),
      calendar.component(.hour, from: calendar.date(from: DateComponents(hour: 23))!),
      "Expected 23 hours.")
  }
}
